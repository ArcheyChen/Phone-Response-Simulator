...
char* phone_response(const char* input_id_str) {
    static char result[256];  // Static buffer for result
    char confirmation_id[49];
    int err = generate(input_id_str, confirmation_id);
    ...
    if (err) {
        const char* error_messages[] = {
            ...
            "Invalid character",
            "Invalid check digit",
            "Unknown version of ID",
            "This ID is really unlucky"
        };
        snprintf(result, sizeof(result), "ERROR: %s", error_messages[err]);
        return result;
    } else {
        snprintf(result, sizeof(result), "%s", confirmation_id);
        return result;
    }
}

void print_usage() {
	printf("Phone Response Simulator\n");
	printf("Usage: phone_response <input_id>\n");
	printf("Example: phone_response \"298011-913686-508043-363303-233555-258502-178954-424100-254921\"\n");
}
...
int main(int argc, char* argv[])
{
	...
	CHARTYPE confirmation_id[49];
	int err = generate(argv[1], confirmation_id);
